...
public class PositionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("/update".equalsIgnoreCase(path)) {
            ...
            String currentStatus = value(existing.get("status")).toLowerCase();
            if ("closed".equals(currentStatus)) {
                out.print(new JSONObject().put("success", false).put("message", "Closed positions cannot be edited").toString());
                return;
            }
            ...
            String before = dataManager.formatPositionSnapshot(existing);
            ...
            String after = dataManager.formatPositionSnapshot(updated);
            dataManager.writeLog(user.getUserId(), user.getUserName(), user.getRole(), "UPDATE_POSITION", positionId + " before=" + before + " after=" + after, "success");
        ...
        if ("/status".equalsIgnoreCase(path) || "/publish".equalsIgnoreCase(path)) {
            ...
            if (publishAction) {
                ...
                String title = value(existing.get("title"));
                dataManager.writeLog(user.getUserId(), user.getUserName(), user.getRole(), "UPDATE_POSITION_STATUS", "Position '" + title + "' (ID: " + positionId + ") status changed from " + value(existing.get("status")) + " to " + normalized, "success");

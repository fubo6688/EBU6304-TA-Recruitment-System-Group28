...
public class DataManager {
    ...
    public synchronized String formatPositionSnapshot(Map<String, String> position) {
        if (position == null) {
            return "";
        }
        return String.format("title=%s;department=%s;salary=%s;requirements=%s;openings=%s;deadline=%s;status=%s",
                safe(position.get("title")),
                safe(position.get("department")),
                safe(position.get("salary")),
                safe(position.get("requirements")),
                safe(position.get("openings")),
                safe(position.get("deadline")),
                safe(position.get("status")));
    }
    ...
    public synchronized boolean updatePosition(String positionId,
        ...
        for (Map<String, String> p : positions) {
            if (positionId.equals(p.get("id"))) {
                String currentStatus = safe(p.get("status")).toLowerCase();

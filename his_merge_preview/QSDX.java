...
public class DataManager {
    ...
    public synchronized List<Map<String, String>> getAllApplications() {
        ...
        for (String line : readLinesSafe(resolve(APPLICATIONS_FILE))) {
            ...
            item.put("resumeAnalysis", p.length >= 11 ? decodeOpaque(p[10]) : "");
            apps.add(item);
        }
        return apps;
    }
    ...
    public synchronized Map<String, String> submitApplication(String userId,
                                                               ...
                                                               String positionId,
                                                               String priority) {
        return submitApplication(userId, userName, positionId, priority, "");
    }

    /**
     * 提交申请并联动岗位 appliedCount 计数，同时保存 AI 简历分析快照。
     */
    public synchronized Map<String, String> submitApplication(String userId,
                                                               String userName,
                                                               String positionId,
    ...
    private String toApplicationLine(Map<String, String> app) {
        return String.join("|",
                ...
                encodeOpaque(app.get("resumeAnalysis")));
    }
    ...
    public synchronized void saveProfile(String userId,
        ...
        for (String line : lines) {
            ...
            if (p.length > 0 && userId.equals(p[0])) {
                String existingAnalysis = p.length >= 13 ? p[12] : "";
     ...
     * 仅更新用户档案中的 AI 简历分析快照。
     */
    public synchronized boolean saveProfileAnalysis(String userId, String resumeAnalysis) {
        List<String> lines = readLinesSafe(resolve(PROFILES_FILE));
        List<String> updated = new ArrayList<>();
        boolean replaced = false;
        String encodedAnalysis = encodeOpaque(resumeAnalysis);

        for (String line : lines) {
            String[] p = line.split("\\|", -1);
            if (p.length > 0 && userId.equals(p[0])) {
                String grade = p.length > 1 ? p[1] : "";
                String major = p.length > 2 ? p[2] : "";
                String gpa = p.length > 3 ? p[3] : "";
                String email = p.length > 4 ? p[4] : "";
                String skills = p.length > 5 ? p[5] : "";
                String resumeFileName = p.length > 6 ? p[6] : "";
                String resumeStoredName = p.length > 7 ? p[7] : "";
                String availableTime = p.length > 8 ? p[8] : "";
                String avatarStoredName = p.length > 9 ? p[9] : "";
                String taExperience = p.length > 10 ? p[10] : "";
                String updatedAt = p.length > 11 ? p[11] : nowDateTime();
                updated.add(String.join("|",
                        safe(userId),
                        safe(grade),
                        safe(major),
        ...
        for (String line : readLinesSafe(resolve(PROFILES_FILE))) {
            ...
            if (p.length >= 8 && userId.equals(p[0])) {
                ...
                if (p.length >= 12) {
                    ...
                    result.put("resumeAnalysis", p.length >= 13 ? decodeOpaque(p[12]) : "");
                } else if (p.length >= 11) {
                    result.put("resumeStoredName", p[7]);
                    result.put("availableTime", p[8]);
                    result.put("avatarStoredName", p[9]);
                ...
                if (!result.containsKey("resumeAnalysis")) {
                    result.put("resumeAnalysis", "");
                }
                return result;
            }
    ...
    /**
     * 将分析 JSON 以 Base64 形式存储，避免破坏管道分隔格式。
     */
    private String encodeOpaque(String raw) {
    ...
    /**
     * 还原 Base64 编码的分析 JSON。
     */
    private String decodeOpaque(String encoded) {
